...
function MapBoundsController({
  ...
  featureCollection: FinepMunicipioMapFeatureCollection;
}) {
  const map = useMap();
...
interface FinepMunicipioResourceMapClientProps {
  featureCollection: FinepMunicipioMapFeatureCollection | null;
  loading: boolean;
}

export default function FinepMunicipioResourceMapClient({
  featureCollection,
  loading,
}: FinepMunicipioResourceMapClientProps) {
  ...
  const topFeatures = useMemo(
    () => [...features]
      ...
      .sort(
        (left, right) =>
          right.properties.total_aprovado_finep - left.properties.total_aprovado_finep
      )
      .slice(0, 5),
    [features]
  );
  ...
  const effectiveSelectedCode = useMemo(() => {
    ...
    const firstFeatureWithValue = features.find(
      (feature) => feature.properties.total_aprovado_finep > 0
    );
  ...
  return (
    <ComponentCard
      title="Mapa de Recursos por Município"
      desc="Polígonos municipais do schema geo coloridos pelo valor aprovado FINEP no recorte atual."
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.75fr)_minmax(320px,0.85fr)]">
        <div className="overflow-hidden rounded-3xl border border-gray-200 bg-[radial-gradient(circle_at_top,#fff7ed_0%,#ffffff_44%,#eff6ff_100%)] p-3 dark:border-gray-800 dark:bg-[radial-gradient(circle_at_top,#3b1d12_0%,#111827_42%,#0f172a_100%)]">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 px-2 pt-1">
            <div>
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">Cobertura geográfica</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {loading
                  ? "Carregando polígonos municipais..."
                  : `${featureCount.toLocaleString("pt-BR")} municípios com geometria, ${municipalitiesWithValue.toLocaleString("pt-BR")} com valor no recorte`}
              </p>
            </div>
            ...
            {features.length ? (
              ...
              <div className="flex h-full items-center justify-center px-6 text-center text-sm text-gray-500 dark:text-gray-400">
                {loading
                  ...
                  : "Nenhum município com geometria correspondente foi encontrado para o filtro atual."}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
              ...
              {topFeatures.map((feature) => (
                ...
                >
                  ...
                  <Badge color="warning" size="sm">
                    {formatCompactMoney(feature.properties.total_aprovado_finep)}
                  </Badge>
                </button>
              ))}
              {!topFeatures.length && !loading && (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Nenhum município com valor no filtro atual.
                </p>
              )}
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-gray-800 dark:bg-gray-900/70">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-medium text-gray-800 dark:text-white/90">Município em foco</p>
              {selectedFeature && <Badge color="success" size="sm">Selecionado</Badge>}
            </div>
            ...
            {selectedFeature ? (
              <div className="mt-4 space-y-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    IBGE {selectedFeature.properties.municipality_ibge_code}
                  </p>
                  <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                    {selectedFeature.properties.total_aprovado_finep > 0 ? "Município com recursos no recorte atual." : "Município sem recursos no recorte atual."}
                  </p>
                </div>
              ...
              <p className="mt-4 text-sm text-gray-500 dark:text-gray-400">
                Selecione um município no mapa para inspecionar o volume de recursos e a cobertura empresarial.
              </p>
            )}
          </div>
        </div>
