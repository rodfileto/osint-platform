...
```
Internet → Cloudflare Edge → cloudflared (container) → frontend:3000 / backend:8000 / keycloak:8080 / airflow-webserver:8080
```
...
CF_AUTH_DOMAIN=auth.yourdomain.com
...
The frontend login redirect depends on the public Keycloak hostname resolving in Cloudflare.
If `KEYCLOAK_PUBLIC_URL` or `VITE_KEYCLOAK_URL` points to `https://auth.yourdomain.com`, you must also create the matching tunnel public hostname for `auth.yourdomain.com` to `http://keycloak:8080`.
If you skip this step, the browser will fail before reaching Keycloak and show a host lookup error such as `Server not found`.
