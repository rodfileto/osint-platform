"""
FINEP Loader — Liberações Crédito Descentralizado
...
Mantém cnpj_proponente para JOIN direto com cnpj.estabelecimento.
Contexto do contrato via JOIN com finep.projetos_credito_descentralizado
usando (contrato).

Mapeamento de colunas (0-based):
  1  → contrato
  3  → cnpj_proponente
  9  → num_parcela         (Numero da Parcela)
  10 → data_liberacao      (Data da Liberacao)
  11 → valor_liberado      (Valor Liberado)
"""
...
INSERT_SQL = """
    INSERT INTO finep.liberacoes_credito_descentralizado (
        contrato, cnpj_proponente,
        num_parcela, data_liberacao, valor_liberado,
        manifest_id
    ) VALUES (
        %(contrato)s, %(cnpj_proponente)s,
        %(num_parcela)s, %(data_liberacao)s, %(valor_liberado)s,
        %(manifest_id)s
    )
"""
...
def iter_rows(xlsx_path: Path):
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=DATA_START, values_only=True):
            ...
            # contrato pode ser numérico no xlsx (ex: 45938) — normalizar para str
            raw_contrato = row[1]
            if raw_contrato is None:
                continue
            contrato = str(int(raw_contrato)).strip() if isinstance(raw_contrato, float) else _clean_str(raw_contrato)
            if not contrato:
                continue
            ...
            yield {
                ...
                "num_parcela":    _clean_smallint(row[9]),
                "data_liberacao": _clean_date(row[10]),
                "valor_liberado": _clean_numeric(row[11]),
                "manifest_id":    None,
            }
    finally:
        wb.close()
...
def load_liberacoes_credito_descentralizado(
    ...
    logger.info(f"Carregando liberacoes_credito_descentralizado | manifest_id={manifest_id} | {xlsx_path}")
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM finep.liberacoes_credito_descentralizado WHERE manifest_id = %s",
                (manifest_id,),
            )
