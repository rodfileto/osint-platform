"""
FINEP Loader — Liberações Ancine
...
Contexto do contrato via JOIN com finep.projetos_ancine
usando (contrato, ref).
...
INSERT_SQL = """
    INSERT INTO finep.liberacoes_ancine (
        contrato, ref,
...
def load_liberacoes_ancine(
    ...
    logger.info(f"Carregando liberacoes_ancine | manifest_id={manifest_id} | {xlsx_path}")
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM finep.liberacoes_ancine WHERE manifest_id = %s",
                (manifest_id,),
            )
