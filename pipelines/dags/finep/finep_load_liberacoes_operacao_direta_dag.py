"""
FINEP Load Liberações Operação Direta DAG
...
Fluxo:
  start ──▶ load_liberacoes_operacao_direta ──▶ end
...
def _load(**context) -> dict:
    from loader_liberacoes_operacao_direta import load_liberacoes_operacao_direta

    conf = context["dag_run"].conf or {}
    result = load_liberacoes_operacao_direta(
...
with DAG(
    dag_id="finep_load_liberacoes_operacao_direta",
    default_args=default_args,
    description="Carga de liberações Operação Direta (Liberacao.xlsx) → PostgreSQL",
    ...
    load_task = PythonOperator(
        task_id="load_liberacoes_operacao_direta",
