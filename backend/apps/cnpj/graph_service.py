...
class CompanyNetworkService:
    ...
    def get_person_network(self, cpf_masked: str, nome: str | None = None, depth: int = 1):
        """
        Retorna a rede de co-propriedade de uma pessoa física centrada no nó :Pessoa
        identificado por cpf_cnpj_socio (mascarado) e opcionalmente nome.

        depth=1 → empresas diretas (1 hop)
        depth=2 → co-sócios nessas empresas (2 hops)
        """
        max_depth = max(1, settings.NEO4J_MAX_NETWORK_DEPTH)
        normalized_depth = max(1, min(depth, max_depth))
        # For person-centred network: each depth level is one hop
        # depth 1 = Pessoa→Empresa; depth 2 = Pessoa→Empresa←Pessoa
